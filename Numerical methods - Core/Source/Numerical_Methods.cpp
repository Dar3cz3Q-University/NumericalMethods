...
{
	double horner(double x, const double* a, uint32_t n)
	{
		double result = a[0];
	...
	double hornerN(double x, const double* b, const double* xn, uint32_t n)
	{
		double result = b[0];
	...
	double lagrange(const Point* nodes, double x, uint32_t n)
	{
		double result = 0;
